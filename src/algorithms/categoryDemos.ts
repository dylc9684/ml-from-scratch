...
const hiddenMarkovModels = makeConceptAlgorithm({
  ...
  engine: (_, params) => {
    const mode = stringParam(params, "hmmMode", "market") === "pos" ? "pos" : "market";
  ...
  python: (params) => {
    const mode = stringParam(params, "hmmMode", "market") === "pos" ? "pos" : "market";
    const text = JSON.stringify(stringParam(params, "hmmInput", mode === "pos" ? "The quick model learns markets" : "Up Down Flat Up"));
...
function parseHmmObservations(mode: "market" | "pos", input: string) {
  if (mode === "market") {
    const tokens = input
      .split(/[\s,>→-]+/)
...
function buildHmmConfig(mode: "market" | "pos") {
  if (mode === "pos") {
    const states = [
...
function runHmmViterbi(mode: "market" | "pos", observations: string[]) {
  const config = buildHmmConfig(mode);
  const trellis = observations.map((observation, time) =>
    config.states.map((state) => {
      ...
      if (time === 0) {
        return {
          ...
          observation,
          logProbability: safeLog(config.start[state.key as keyof typeof config.start]) + emissionLog,
          emissionLog,
          transitionLog: safeLog(config.start[state.key as keyof typeof config.start]),
      ...
      const candidates = trellis[time - 1].map((previousCell) => {
        const transitionProbability =
          config.transition[previousCell.stateKey as keyof typeof config.transition]?.[state.key] ?? 1e-6;
